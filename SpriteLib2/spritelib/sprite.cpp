...
namespace spritelib
{
	...
	*/
	...
	void Sprite::HorizontalScroll(float amount)
	{
		float scrollingSpeed = amount;

		// update our x position by scrollingSpeed pixels
		set_position(get_position().x - scrollingSpeed, get_position().y);

		// check if we've gone too far
		if (get_position().x + get_scale().x <= Window::get_game_window().get_width(true))
			set_position(0.0f, get_position().y);
		
		if (get_position().x > 0)
			set_position(-Window::get_game_window().get_width(true), get_position().y);
	}

	Sprite & Sprite::load_sprite_image(std::string a_fileLoc, bool a_setScale)
	{
		//only function that uses m_loc so it is safe to change:
		m_loc = a_fileLoc;
		if (m_images.find(a_fileLoc) == m_images.end())
		{
			sf::Image upsideDownImage;
			upsideDownImage.loadFromFile("assets/images/" + a_fileLoc + ".png");
	...
	*/
	...
	Sprite & Sprite::next_frame2()
	{
		m_currentFrame++;
		if (m_currentFrame > m_currentAnimation.size() - 1)
			m_currentFrame = 1;
		return *this;
	}
	/* draw
	...
	*/
	Sprite & Sprite::draw()
	{
		...
		if (m_animations.find(m_currentAnimation) == m_animations.end())	//end means it didn't find the element (nullptr)
		{
