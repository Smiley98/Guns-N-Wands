...
namespace spritelib
{
	...
	Window & Window::set_ortho_matrix(float a_x, float a_y, float a_width, float a_height) {
		//*m_proj = math::Matrix4x4(math::Matrix4x4::ortho(a_x, a_x + a_width, a_y, a_y + a_height, -100.0f, 100.0f).m_data);
		*m_proj = math::Matrix4x4::ortho(a_x, a_x + a_width, a_y, a_y + a_height, -100.0f, 100.0f);
		return *this;
	}
	...
	bool Window::update(int a_targetFPS)
	{
		...
		while (m_window->pollEvent(event))
		{
			...
			if (event.type == 11)
			{
				m_mouseFunction(Button::UnknownButton,	//inverting y to be 0 at bottom and height at top
