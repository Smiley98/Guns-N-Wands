...
namespace spritelib
{
	...
	{
		if (m_uniformLocations.find(a_location) == m_uniformLocations.end())	//if the search gets to the end of the map without finding the specified value,
		{																		//add it to the list of ACTIVE uniform variables (they all there, just no point in loading them all if not all get used
			m_uniformLocations[a_location] = glGetUniformLocation(m_shaderProg, &a_location[0]);	//finds and stores the location as an integer (array index) of the specified uniform.
		}
